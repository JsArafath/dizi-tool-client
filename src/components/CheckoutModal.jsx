...
export default function CheckoutModal({ isOpen, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.paymentMethod === 'bkash' || formData.paymentMethod === 'nagad') {
        if (!formData.trxId) {
          setError(`Please enter your ${formData.paymentMethod === 'bkash' ? 'bKash' : 'Nagad'} Transaction ID (TrxID)`);
          setLoading(false);
          return;
        }
  ...
  const bkashLogo = "https://upload.wikimedia.org/wikipedia/commons/thumb/u/u8/BKash_Logo.svg/512px-BKash_Logo.svg.png";
  const nagadLogo = "https://upload.wikimedia.org/wikipedia/en/thumb/v/va/Nagad_Logo.svg/512px-Nagad_Logo.svg.png";
        ...
        <form onSubmit={handleSubmit} className="checkout-form">
            ...
            <div className="payment-methods">
              <label className={`payment-method-option ${formData.paymentMethod === 'bkash' ? 'selected' : ''}`}>
                <input 
                  ...
                  onChange={handleChange} 
                />
                <img src={bkashLogo} alt="bKash" height="30" />
                <span>bKash</span>
              </label>
              <label className={`payment-method-option ${formData.paymentMethod === 'nagad' ? 'selected' : ''}`}>
                <input 
                  ...
                  onChange={handleChange} 
                />
                <img src={nagadLogo} alt="Nagad" height="30" />
                <span>Nagad</span>
              </label>
            </div>
          </div>
          ...
          {formData.paymentMethod === 'bkash' && (
            <div style={{ background: '#e2136e0d', border: '1px dashed #e2136e', padding: '20px', borderRadius: '12px', marginBottom: '20px', textAlign: 'center' }}>
              <img src={bkashLogo} alt="bKash" height="35" style={{ marginBottom: '15px' }} />
          ...
          {formData.paymentMethod === 'nagad' && (
            <div style={{ background: '#fdf3ed', border: '1px dashed #ed1c24', padding: '20px', borderRadius: '12px', marginBottom: '20px', textAlign: 'center' }}>
              <img src={nagadLogo} alt="Nagad" height="35" style={{ marginBottom: '15px' }} />
              <p style={{ margin: '0 0 10px 0', fontSize: '15px', color: '#444', lineHeight: '1.5' }}>
                অনুগ্রহ করে নিচের নাম্বারে <strong>৳{totalBdt}</strong> Send Money করুন:
              </p>
              <div style={{ background: '#fff', display: 'inline-block', padding: '10px 20px', borderRadius: '8px', boxShadow: '0 2px 8px rgba(237, 28, 36, 0.15)', marginBottom: '15px' }}>
                <h3 style={{ margin: 0, color: '#ed1c24', fontSize: '24px', letterSpacing: '1px', userSelect: 'all' }}>01879009680</h3>
              </div>
              <p style={{ margin: '0 0 15px 0', fontSize: '13px', color: '#666' }}>
                টাকা পাঠানোর পর আপনার Nagad Transaction ID (TrxID) নিচে প্রদান করুন।
              </p>
              ...
              <div style={{ textAlign: 'left' }}>
                <input 
                  ...
                  value={formData.trxId || ''} 
                  onChange={handleChange} 
                  placeholder="Enter Nagad TrxID (e.g. 7JA7H8P3N9)"
                  style={{ width: '100%', padding: '12px', border: '1px solid #ddd', borderRadius: '6px', outline: 'none', fontSize: '14px', boxSizing: 'border-box' }}
                />
              </div>
            </div>
